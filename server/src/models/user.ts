...
export function createUser(sessionId : string, username : string, now :number, roomCode : string, isModerator : boolean) {
    connection.query('INSERT INTO User(sessionId, username, createdAt , roomId, isModerator, state, vote) VALUES (?, ?, ?, ?, ?, ?, ?)', [sessionId, username, now, roomCode, isModerator, "voting", ""], (err, rows) => {
        if(err) throw err;
    });
}
...
export function getUsersInRoom(roomCode : string) : Promise<User[]> { //Used to display all users
    return new Promise((resolve, reject) => {
...
export function giveUserModeratorRights(sessionId : string) {
    connection.query('UPDATE User SET isModerator = 1 WHERE sessionId = ?', [sessionId], (err, rows) => {
            if(err) throw err;
    });
}
